...
| Declares an earlier version, or none, on a gateway without one | the policy parses the request body itself | no |
...
**A request whose operation cannot be identified is authenticated, never exempted.** The exception
lists say which operations skip authentication, so an operation the gateway could not determine
matches nothing and is treated as requiring a token. Two cases reach this:

- a modern request that omits `Mcp-Method`, or omits `Mcp-Name` on `tools/call`, `resources/read` or
  `prompts/get` — the specification requires those, and without them the headers describe nothing the
  exception lists can be matched against;
- a request whose body the gateway could not read at all, which is rejected outright.
...
attach the **mcp-validation** policy ahead of this one; without it, the MCP server is responsible for
rejecting a request whose headers and body disagree.
