...
The **MCP Validation** policy rejects malformed and self-contradictory MCP requests at the
...
The policy runs during the request-header phase and does not read the request body itself — the
gateway parses the body once for the whole policy chain, and this policy works from what that
parse found.
...
The proxy's route must carry the gateway's MCP operation resolver, which the gateway adds to an MCP
proxy's request endpoint automatically. A request on a route without one is rejected rather than
silently forwarded unvalidated.
...
```yaml
- name: mcp-validation
  gomodule: github.com/wso2/gateway-controllers/policies/mcp-validation@v0
```
