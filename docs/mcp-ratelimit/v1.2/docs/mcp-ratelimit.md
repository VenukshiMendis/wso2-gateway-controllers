...
`Mcp-Method` and `Mcp-Name` agree with the request body. Attach the [MCP Validation
policy](../../../mcp-validation/v0.1/docs/mcp-validation.md) ahead of this one to close that: it
requires the mirrored headers and checks them against the body, rejecting a mismatch with JSON-RPC
`-32020`. Without it, the MCP server is responsible for that check.
