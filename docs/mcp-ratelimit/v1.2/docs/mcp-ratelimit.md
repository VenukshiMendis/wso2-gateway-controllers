...
|---|---|---|
| Declares `MCP-Protocol-Version: 2026-07-28` or later | the mirrored `Mcp-Method` and `Mcp-Name` headers | no |
...
| Declares an earlier version, or none, on a gateway without one | the policy reads the request body itself | no |
...
- a JSON-RPC **response**, which a client POSTs to answer a server-initiated sampling or elicitation
  call and which carries an id and a result but no method;
- a modern request that omitted `Mcp-Method`, or whose `Mcp-Name` could not be decoded.

⚠️ **The last case is worth knowing about.** On a request declaring MCP 2026-07-28 this policy reads
the mirrored headers and does not check them against the body. A caller that omits `Mcp-Method`, or
sends an `Mcp-Name` that will not decode, therefore reaches a capability this policy would otherwise
have limited — and because a `tools`, `resources` or `prompts` rule is matched on the capability
name, an undecodable name escapes those rules even when they are written as `"*"`. A `methods` rule
still applies, since it matches on the method alone.

Attach the [MCP Validation policy](../../../mcp-validation/v0.1/docs/mcp-validation.md) ahead of this
one to close that: it requires the mirrored headers and checks them against the body, rejecting a
mismatch with JSON-RPC `-32020`. Without it, the MCP server is responsible for that check.
