...
MCP `2026-07-28` makes mirroring mandatory, so both headers must be present and must agree with
the body. `Mcp-Method` is checked before the capability is resolved; `Mcp-Name` is checked as soon
as the request names one, which by that point is always a `tools/call`, `resources/read` or
`prompts/get` — the three operations that target a capability and must therefore mirror it. A
modern request that omits either header, or that names something the body contradicts, is
rejected with `-32020`. `Mcp-Method` is never corrected, because a rewrite never changes the
method.
...
If MCP Validation is attached, declare it first of all.
