...
`Mcp-Method` is checked before the capability is resolved, `Mcp-Name` as soon as the body names
one. Neither is corrected when it is missing or wrong: a rewrite changes only the capability
name, so a request whose mirror cannot be trusted is refused rather than repaired. Deciding
whether a modern request was *obliged* to send a header in the first place is the MCP Validation
policy's responsibility, not this one's.
