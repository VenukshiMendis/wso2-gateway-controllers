...
the mirrored headers and does not check them against the body. A caller that omits `Mcp-Method`
therefore reaches a capability this policy would otherwise have governed. Attach the
[MCP Validation policy](../../../mcp-validation/v0.1/docs/mcp-validation.md) ahead of this one to
close that: it requires the mirrored headers and checks them against the body, rejecting a mismatch
with JSON-RPC `-32020`. Without it, the MCP server is responsible for that check.
