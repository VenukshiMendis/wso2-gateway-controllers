...
⚠️ **On a gateway without a resolver this check does not run**, and a body that names a member
more than once — including under a different letter case, such as `method` and `Method` — may be
read as one operation by this policy and executed as another by the MCP server. Attach the
[MCP Validation policy](../../../mcp-validation/v0.1/docs/mcp-validation.md) ahead of this one to
close that, or deploy on a gateway whose routes carry the operation resolver.
