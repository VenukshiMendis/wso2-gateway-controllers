...
// by hasMCPResolver, since Mode() is read once at chain-build time.
...
func TestHasMCPResolver(t *testing.T) {
	for _, tc := range []struct {
		...
		{"a resolver, but not on an MCP route", &policy.SharedContext{ResolvedOperation: "a2a"}, false},
		{"the MCP resolver", &policy.SharedContext{APIKind: policy.APIKindMCP, ResolvedOperation: "mcp"}, true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			if got := hasMCPResolver(tc.shared); got != tc.want {
				t.Errorf("hasMCPResolver = %v, want %v", got, tc.want)
			}
		})
	}
}
...
// about the decision and the rules apply as usual. Rejecting that body is mcp-validation's job.
