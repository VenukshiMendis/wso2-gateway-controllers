...
//
// Checking that a modern request's headers describe its body is mcp-validation's job.
package mcpauthn
...
// whether the route carries the MCP resolver, so hasMCPResolver settles which phase decides.
...
func (p *McpAuthPolicy) OnRequestHeaders(ctx context.Context, reqCtx *policy.RequestHeaderContext, params map[string]interface{}) policy.RequestHeaderAction {
	...
	if isMcpPostRequest(ds.Method, reqCtx.OperationPath) {
		if !hasMCPResolver(reqCtx.SharedContext) {
...
// the headers-match-body check are both mcp-validation's job.
