...
// Package mcpvalidation validates MCP requests: the body on every request, and the mirrored
...
// McpValidationPolicy holds no state: it takes no parameters, and one instance is shared
...
func (p *McpValidationPolicy) OnRequestHeaders(
	...
	if !strings.EqualFold(reqCtx.Method, http.MethodPost) || !hasMCPResolver(reqCtx.SharedContext) {
		return nil
	}
...
func (p *McpValidationPolicy) OnRequestBody(
	_ context.Context,
...
// mcpResolverOperation is the operation the MCP resolver sets on every route it serves.
const mcpResolverOperation = "mcp"

// hasMCPResolver reports whether the MCP resolver read this request's body.
func hasMCPResolver(shared *policy.SharedContext) bool {
	return shared != nil &&
		shared.APIKind == policy.APIKindMCP &&
		shared.ResolvedOperation == mcpResolverOperation
}
