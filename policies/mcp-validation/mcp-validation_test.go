...
func runBody(t *testing.T, p *McpValidationPolicy, reqCtx *policy.RequestContext) policy.RequestAction {
	t.Helper()
...
func TestReadableBodyIsForwardedWithoutTheResolver(t *testing.T) {
	...
	for name, body := range map[string]string{
		...
		"a params member of the wrong type": `{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{"name":42}}`,
...
func TestUnusableBodyReasonMatchesTheResolver(t *testing.T) {
	for body, want := range map[string]string{
		...
		`{"params":{"clientInfo":{},"ClientInfo":{}}}`:             reasonAmbiguous,
